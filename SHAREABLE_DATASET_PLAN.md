...
| per-SNP dosage GWAS geno | (from dosage) "persnp" | 0..2 continuous | raw, no ancestry smoothing |
...
| Raw per-SNP teosinte dosage (no smoothing) | **`persnp`** (or "per-SNP dosage") | fine as-is; just document it |
