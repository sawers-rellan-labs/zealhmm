...
| `teonam_gwas118k_v5_dosage_polar` | 118,838 | v5 | dosage **polarized to W22↔teosinte** (0=W22, 2=teo; teosinte=minor allele, 0 flips, 99.6% concordant with 51K coding), from the distributed imputed-**nucleotide** genotypes. **NOT the sweep truth** — its ancestry is choppy (~889 flips/chr per RIL = scattered per-SNP imputation het, not the ~3 breakpoints of a real BC1S4 RIL); use only for direct genotype work. `teonam_gwas118k_dosage_polar.rds` (`scripts/teonam_gwas118k_polarize.R`) |
| `teonam_gwas118k_v5_dosage_fsfhap` | 118,514 | v5 | **the sweep truth** — the 118K block-smoothed with TASSEL5 **FSFHap** exactly as Chen (bc, Phet=0.03125, Fillgaps, per family), taking FSFHap's **parental (A/C) output** (clean ancestry mosaic, ~9 breakpoints/chr) and step-interpolating each family onto the union grid → dense clean-block 118K. `teonam_gwas118k_dosage_fsfhap.rds` (`scripts/teonam_gwas118k_fsfhap.R` + `teonam_gwas118k_truth_assemble.R`). *Why:* the distributed 118K hmp is imputed nucleotide genotypes, not FSFHap's parental calls, so converting it to ancestry is choppy; re-taking the parental output restores the clean mosaic the sweep needs. |
...
| `teonam_gwas118k_v5_<caller>_sweep` | 118,514 | v5 | **coverage-degradation sweeps** — reads simulated from the **FSFHap clean-block truth** (`_dosage_fsfhap`) per family, ancestry-called, interpolated on the 118K cM grid, STAM-scanned at λ∈{0.1,0.2,0.5,1,5,10,20,Inf}. **λ=Inf baseline = GWAS on the complete FSFHap truth matrix** (`scripts/teonam_sweep_baseline_118k.R` → `stam_gwas_scan_118k_complete_baseline.csv`), NOT the authentic per-SNP scan (whose scatter breaks the line-Manhattan). Four callers: `teonam_{rtiger,nnil,lbimpute,control}_sweep_118k.R` → `results/sim/teonam/stam_gwas_<caller>_118k_sweep.csv`. Same models/params/seeds as the 51K siblings. |
