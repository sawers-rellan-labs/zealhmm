...
| `teonam_gwas118k_v5_<caller>_sweep` | 118,514 | v5 | **coverage-degradation sweeps on 118K truth** — reads simulated from the dense polarized 118K per family, ancestry-called, interpolated on the 118K cM grid, STAM-scanned at λ∈{0.1,0.2,0.5,1,5,10,20,Inf}, λ=Inf baseline = the authentic 118K scan. Four callers, one script each: `teonam_{rtiger,nnil,lbimpute,control}_sweep_118k.R` → `results/sim/teonam/stam_gwas_<caller>_118k_sweep.csv`. Same models/params/seeds as the 51K siblings (`teonam_*_sweep.R`), only truth+grid+baseline swapped. |
