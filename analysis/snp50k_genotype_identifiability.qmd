...
subtitle: "The caller's rare-allele prior and outbred-donor dilution — with the panel and read data"
...
`--min-af 0.05:minor` and intersected to the ZEAL HQ sites. **Not imputed.** So
panel sites are *common* teosinte-vs-B73 variants — segregating in the taxon,
...
Per-donor-taxon genotype composition at panel sites (217-sample teosinte panel;
each row is the per-individual average):

| donor taxon | f₀ (REF) | f₁ (HET) | f₂ (ALT-hom) | het\|non-ref | **d = f₂ + ½·f₁** |
|---|---:|---:|---:|---:|---:|
...
2. **The read-based $d$ reproduces the panel-genotype $d$** for the four
   ...
   **d ≈ 0.3, not ≈ 1.** The reads run slightly high, consistent with
   (i) the specific donor accessions carrying more alt than the taxon average and
...
`--min-af 0.05:minor` taxon panel identifies *common* variants, not
...
- **Prefer donor-enriched sites where possible.** Selecting sites with high
  taxon-level $f_2$ raises $d$ toward 1 and widens state separation, up to the
...
# §2.3 per-taxon donor dose d = f2 + 0.5*f1  (ref_<taxon>.txt = panel sample lists)
