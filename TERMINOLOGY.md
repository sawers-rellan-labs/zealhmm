...
|------|---------|-----------------------------|
| **chip-supervised GBS calibration** (of the HMM) | Tuning the GBS caller's parameters to minimize disagreement between its GBS calls and the chip calls on the 24 both-platform lines. **GBS is the sole HMM input; the chip supplies reference labels.** This is Holland's File_S16. | Deprecated labels: "joint calibration", "GBS ∪ chip", **"chip∩GBS"** (and any union/intersection set-framing). Chip is *supervision labels*, never a data set combined with GBS. It is **not** a union co-fit (chip genotypes are never stacked as input) and **not** Kennedy-O'Hagan (no simulator). |
| **chip-supervised markers** | The markers where chip labels exist (the SNP-chip positions also typed in the GBS; ~11,310 on the 24 lines). Where any chip-supervised score/comparison is evaluated. | Say "chip-supervised markers", never "chip∩GBS". Distinct from the FULL GBS marker set (~63,904) the published dense calls run on. |
