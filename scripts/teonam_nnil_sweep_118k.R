...
for (li in seq_along(LAMBDAS)) {
  ...
  fwrite(scan, file.path(OUTDIR, sprintf("%s_gwas_nnil_118k_lambda%s.csv", TTAG, lambda)))
...
fwrite(rbindlist(sweep_list, use.names = TRUE), file.path(OUTDIR, sprintf("%s_gwas_nnil_118k_sweep.csv", TTAG)))
fwrite(rbindlist(mlm_list, use.names = TRUE), file.path(OUTDIR, sprintf("%s_gwas_nnil_118k_mlm_sweep.csv", TTAG)))
