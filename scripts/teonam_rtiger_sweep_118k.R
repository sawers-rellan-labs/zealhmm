...
for (li in seq_along(LAMBDAS)) {
  ...
  fwrite(scan, file.path(OUTDIR, sprintf("%s_gwas_rtiger_118k_lambda%s.csv", TTAG, lambda)))
  scan[, coverage := lambda]
  sweep_list[[li]] <- scan
  null_li <- readRDS(file.path(ROOT, sprintf("data/teonam/mlm_null%s_118k_l%s.rds", NTAG, lambda))) # coverage-matched Family+K (GL-dosage of downsampled reads)
...
fwrite(rbindlist(sweep_list, use.names = TRUE), file.path(OUTDIR, sprintf("%s_gwas_rtiger_118k_sweep.csv", TTAG)))
fwrite(rbindlist(mlm_list, use.names = TRUE), file.path(OUTDIR, sprintf("%s_gwas_rtiger_118k_mlm_sweep.csv", TTAG)))
