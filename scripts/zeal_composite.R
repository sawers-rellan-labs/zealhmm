...
mosaic <- fread(here(sprintf("data/zeal/%s_gwas_mlm_mosaic_%s_snp50k.csv", TTAG, FAMCOL))) # Panel C

# A: JLM lollipop (model-independent QTL; context scan = the mosaic MLM)
p_lolli <- plot_lollipop(
  here(sprintf("results/sim/zeal/tassel/%s_jlm_native1.txt", TTAG)), mosaic,
...
# C: RTIGER ancestry-imputed mosaic MLM (block-smoothed; well-calibrated)
p_C <- plot_manhattan(mosaic,
  sprintf("ZEAL/BZea %s — MLM (Taxon+K), RTIGER ancestry mosaic (lambda_GC = %.2f)", TRAIT, lam(mosaic$P)),
  overlap_csv = overlap, out_png = file.path(OUT, sprintf("%s_gwas_mlm_mosaic_manhattan_snp50k.png", TTAG))
)
