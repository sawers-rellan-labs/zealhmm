...
gt <- fread(here(sprintf("data/zeal/%s_gwas_mlm_ml_gt_%s_snp50k.csv", TTAG, FAMCOL))) # Panel B
...
# B: per-SNP ML genotype MLM (sharp SNP peaks; sparse -> lambda-deflated at 0.4x)
p_B <- plot_manhattan(gt,
  sprintf("ZEAL/BZea %s — MLM (Taxon+K), per-SNP ML genotype (lambda_GC = %.2f)", TRAIT, lam(gt$P)),
  overlap_csv = overlap, out_png = file.path(OUT, sprintf("%s_gwas_mlm_ml_gt_manhattan_snp50k.png", TTAG))
)
