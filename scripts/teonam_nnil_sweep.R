...
#    which fits its own start freqs). design_priors() has no "BC1S4" entry, and
#    the calibration was on BC2S2 SIM NILs; TeoNAM is BC1S4 with the Chen 2019
#    observed frequencies ~8% het / ~15% teosinte-hom, so f_1 = 0.08, f_2 = 0.15.
...
for (li in seq_along(LAMBDAS)) {
  ...
  fwrite(scan, file.path(OUTDIR, sprintf("stam_gwas_nnil_lambda%s.csv", lambda)))
...
fwrite(sweep, file.path(OUTDIR, "stam_gwas_nnil_sweep.csv"))
message(sprintf(
  "wrote %s (%d rows, %d coverage levels)",
  file.path(OUTDIR, "stam_gwas_nnil_sweep.csv"),
  nrow(sweep), uniqueN(sweep$coverage)
))

het <- rbindlist(het_list)
fwrite(het, file.path(OUTDIR, "stam_nnil_het_fraction.csv"))
message("wrote stam_nnil_het_fraction.csv:")
print(het)
