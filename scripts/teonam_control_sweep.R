...
for (li in seq_along(LAMBDAS)) {
  ...
  fwrite(scan, file.path(OUTDIR, sprintf("stam_gwas_control_native_lambda%s.csv", lambda)))
...
fwrite(sweep, file.path(OUTDIR, "stam_gwas_control_native_sweep.csv"))
message(sprintf(
  "wrote %s (%d rows, %d coverage levels)",
  file.path(OUTDIR, "stam_gwas_control_native_sweep.csv"),
  nrow(sweep), uniqueN(sweep$coverage)
))

het <- rbindlist(het_list)
fwrite(het, file.path(OUTDIR, "stam_control_native_het_fraction.csv"))
message("wrote stam_control_native_het_fraction.csv:")
print(het)
